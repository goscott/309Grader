...
public class UserLoginController {
    ...
    /**
     * Handles the login button click. If login is successful, 
     * launches the MainPage view.
     */
    @FXML
    public void login() {
    ...
    private void launchMainPage() {
        try {
            Stage stage = GraderPopup
                    .getPopupStage("view/mainpage/MainPage.fxml");
            ...
            ScrollBar table1Scroller = findHorizontalScrollBar(GradebookController
                    .get().getMainTable());
            ScrollBar table2Scroller = findHorizontalScrollBar(GradebookController
                    .get().getStatsTable());

            table1Scroller.valueProperty().bindBidirectional(
                    table2Scroller.valueProperty());
            closeLoadScreen();
        }
    ...
    private ScrollBar findHorizontalScrollBar(TableView<?> table) {
        ...
        for (final Node subNode : vf.getChildrenUnmodifiable()) {
            if (subNode instanceof ScrollBar
                    && ((ScrollBar) subNode).getOrientation() == 
    ...
    private void launchNewUser() {
        try {
            Stage stage = GraderPopup.getPopupStage("Create New User",
                    "view/administration/new_user.fxml");
            stage.setOnHiding(new EventHandler<WindowEvent>() {
                public void handle(WindowEvent event) {
