...
public class PermissionsEditorController {
    ...
    public void editPermissions() {
        ...
        user_id.setText(((String) user_list.getSelectionModel()
                .getSelectedItem()).split(" ")[1]);
        user_first_last.setText(users.get(user_id.getText()).getfName() + " "
                + users.get(user_id.getText()).getlName());
        select_permissions.setText(UserTypes.fullName(users.get(
                user_id.getText()).getType()));
    ...
    public void search() {
        ...
        if (search_field.getText().equals("")) {
            ...
            for (User target : users.getUsers()) {
                if (target.getId().toLowerCase()
                        .startsWith(search_field.getText().toLowerCase())
                        || target
                                .getfName()
                                .toLowerCase()
                                .startsWith(
                                        search_field.getText().toLowerCase())
                        || target
                                .getlName()
                                .toLowerCase()
                                .startsWith(
                                        search_field.getText().toLowerCase())
                        || UserTypes.fullName(target.getType()).toLowerCase()
                                .equals(search_field.getText().toLowerCase())) {
                    list.add(String.format(SPACING, target.getId(),
                            UserTypes.fullName(target.getType())));
                }
            }
    ...
    private void refreshKeysAvailable() {
        ObservableList<String> available_list = FXCollections
                .observableArrayList();
