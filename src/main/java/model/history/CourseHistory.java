...
public class CourseHistory implements Serializable {
    ...
    /**
     * All the rosters (different sections from different years) 
     * for this course.
     */
    ...
    //The course does not already exist in history
        ...
        (
            \forall Roster roster_other ;
                history.contains(roster_other) <==>
                    course_other.equals(newRoster) || 
                        \old(history).contains(roster_other)
        );
    @*/
    public void addRoster(Roster newRoster) {

        for (Roster rost : history) {
            if (newRoster.getSection() == rost.getSection()
                    && newRoster.getQuarter() == rost.getQuarter()
                    && newRoster.getStartDate().get(Calendar.YEAR) == rost
                            .getStartDate().get(Calendar.YEAR)) {
                Debug.log("History- Error",
                        "Tried to add a course that is already in the history");
                return;
            }
        }
        ...
        if (!quarters.contains(newRoster.getQuarter() + " "
                + newRoster.getStartDate().get(Calendar.YEAR))) {
    ...
    /*@
        ...
        //history contains only courses that were in 
        //the old version and are not the target
        (
            \forall Roster roster_other ;
                history.contains(roster_other) <==> 
                    !course_other.equals(targetCourse) && 
                        \old(history).contains(roster_other)
            ...
            \forall Roster roster_other2 ;
                \old(history).contains(roster_other2) ==>
                    history.contains(roster_other2) || 
                        course_other2.equals(targetCourse)
        );
    @*/
        ...
        (
            ...
            \forall Roster course_other1;
                hidden.contains(course_other1) <==> 
                    course_other1.equals(targetCourse) || 
                        \old(hidden).contains(course_other1)
            &&
            ...
            \forall Roster course_other2 ;
                history.contains(course_other2) <==>
                    !course_other2.equals(targetCourse) && 
                        \old(history).contains(course_other2)
        ...
        (
            ...
            \forall Roster course_other1 ;
                history.contains(course_other1) <==>
                    course_other1.equals(targetCourse) || 
                        \old(history).contains(course_other1)
            &&
            
            \forall Roster course_other2 ;
                hidden.contains(course_other2) <==>
                    !course_other2.equals(targetCourse) && 
                        \old(hidden).contains(course_other2)
    
            &&
    
            \forall Roster course_other3 ;
                \old(hidden).contains(course_other3) ==>
                    hidden.contains(course_other3) || 
                        course_other3.equals(targetCourse)
        );
    @*/
    ...
    public ArrayList<QuarterAverage> getAverages() {
        ...
        for (String quarter : quarters) {
            ...
            for (Roster roster : history) {
                if ((roster.getQuarter() + " " + roster.getStartDate().get(
                        Calendar.YEAR)).equals(quarter)) {
                    count++;
                    sum += roster.getPercentAverage();
                }
            }
            ...
            if (count > 0) {
                list.add(new QuarterAverage(quarter.split(" ")[0], Integer
                        .parseInt(quarter.split(" ")[1]), sum / count));
            }
        }
    ...
    /**
     * Returns the number of students in the history with a grade 
     * of gradeLetter.
     */
    /*@
          ensures (\result >= 0);
        ...
        for (Roster roster : history) {
            if (roster.getCurve() != null) {
                ...
                try {
                    ...
                    Debug.log("CourseHistory.java ERROR",
                            "Could not find grade " + gradeLetter + " in "
                                    + roster.courseName());
                }
            }
        }
